# Typography & Layout Research for VisualDebugger UI Overhaul

## Executive Summary

The current design uses DM Sans with flat 16px spacing, 4px radii, and standard VS Code card borders. It is functional but generic. This document provides exact specifications for a premium typography system, spacing scale, card treatments, and layout patterns that will transform VisualDebugger from "adequate sidebar" to "polished developer tool."
...
```css
code, .code-block, .prompt-block, .divd-view {
...
:root {
  ...
  --vd-text-xs:    0.694rem;   /* 9.0px  -- metadata, timestamps, heatmap labels */
  --vd-text-sm:    0.833rem;   /* 10.8px -- captions, badge text, secondary labels */
  --vd-text-base:  1rem;       /* 13px   -- body text (matches VS Code default) */
  --vd-text-md:    1.2rem;     /* 15.6px -- section headings, card titles */
  --vd-text-lg:    1.44rem;    /* 18.7px -- panel headings, primary headings */
  --vd-text-xl:    1.728rem;   /* 22.5px -- dashboard title, hero numbers */
  --vd-text-2xl:   2.074rem;   /* 27px   -- stat values, large display numbers */
  ...
  /* ── Line Heights ── */
  --vd-leading-tight:   1.25;  /* headings */
  --vd-leading-snug:    1.4;   /* subheadings, compact text */
  --vd-leading-normal:  1.6;   /* body text */
  --vd-leading-relaxed: 1.75;  /* educational/explanatory text (current vd-body) */

  /* ── Letter Spacing ── */
  --vd-tracking-tight:  -0.02em;  /* large headings */
  --vd-tracking-normal:  0;       /* body text */
  --vd-tracking-wide:    0.04em;  /* uppercase labels, section titles */
  --vd-tracking-wider:   0.06em;  /* small caps, metadata */
}
```
...
h2, .vd-heading {
  ...
  letter-spacing: var(--vd-tracking-normal);
...
:root {
  ...
  --vd-space-0:    0px;
  --vd-space-1:    4px;     /* Icon-to-text gap, inline padding */
  --vd-space-2:    8px;     /* Tight element gap, button padding-y */
  --vd-space-3:    12px;    /* Standard element gap, card padding (compact) */
  --vd-space-4:    16px;    /* Card padding (normal), section gap */
  --vd-space-5:    20px;    /* Section padding, generous card padding */
  --vd-space-6:    24px;    /* Section margin, panel padding */
  --vd-space-8:    32px;    /* Large section breaks */
  --vd-space-10:   40px;    /* Panel top padding, major separations */
  --vd-space-12:   48px;    /* Empty state padding */
}
```
...
|-------|-------|----------|
| `--vd-space-1` | 4px | Icon gaps, badge internal padding (vertical), compact inline spacing |
| `--vd-space-2` | 8px | Button padding-y, gap between badge and text, quiz option gap, heatmap cell gap |
| `--vd-space-3` | 12px | Card padding (compact mode), button padding-x, checklist item padding, body padding on narrow screens |
| `--vd-space-4` | 16px | Card padding (standard), section bottom margin, dashboard grid gap, vd-section padding |
| `--vd-space-5` | 20px | Section bottom margin (major), body padding (standard), vd-panel padding |
| `--vd-space-6` | 24px | Dashboard card padding, generous section spacing, gap between major content blocks |
| `--vd-space-8` | 32px | Panel top padding, large gaps between distinct content areas |
| `--vd-space-10` | 40px | Empty state top padding, hero/splash areas |
| `--vd-space-12` | 48px | Empty state vertical padding, major panel spacing |
...
body {
  padding: var(--vd-space-5);   /* 20px */
}

/* Card padding -- standard content card */
.card {
  padding: var(--vd-space-4);            /* 16px */
  margin-bottom: var(--vd-space-4);      /* 16px */
}

/* Section padding -- vd-section with left border accent */
.vd-section {
  padding: var(--vd-space-3) var(--vd-space-4);  /* 12px 16px */
  padding-left: var(--vd-space-5);                /* 20px (room for border accent) */
  margin-bottom: var(--vd-space-5);               /* 20px */
}

/* Dashboard grid */
.dashboard-grid {
  gap: var(--vd-space-4);                /* 16px */
  margin-bottom: var(--vd-space-6);      /* 24px */
}

/* Checklist items */
.vd-check-item {
  padding: var(--vd-space-2) var(--vd-space-2);   /* 8px */
  gap: var(--vd-space-3);                          /* 12px */
}

/* Button padding */
.btn {
  padding: var(--vd-space-2) var(--vd-space-3);    /* 8px 12px */
}

/* Empty state */
.vd-empty {
  padding: var(--vd-space-12) var(--vd-space-5);   /* 48px 20px */
}

/* Heading bottom margin */
h1 { margin-bottom: var(--vd-space-1); }   /* 4px -- followed by subtitle/description */
h2 { margin-bottom: var(--vd-space-2); }   /* 8px */
h3 { margin-bottom: var(--vd-space-2); }   /* 8px */

/* Paragraph spacing */
p { margin-bottom: var(--vd-space-3); }     /* 12px */

/* Badge padding */
.badge {
  padding: 2px var(--vd-space-2);           /* 2px 8px */
}
```
...
:root {
  /* Card surface tokens */
  --vd-surface-primary: var(--vscode-editor-background);
  --vd-surface-raised: color-mix(in srgb, var(--vscode-editor-background) 95%, var(--vscode-foreground) 5%);
  --vd-surface-overlay: color-mix(in srgb, var(--vscode-editor-background) 90%, var(--vscode-foreground) 10%);
  --vd-border-subtle: color-mix(in srgb, var(--vscode-widget-border) 60%, transparent 40%);
  --vd-border-default: var(--vscode-widget-border);

  /* Elevation tokens */
  --vd-shadow-sm: 0 1px 2px rgba(0, 0, 0, 0.06);
  --vd-shadow-md: 0 2px 8px rgba(0, 0, 0, 0.08), 0 1px 2px rgba(0, 0, 0, 0.04);
  --vd-shadow-lg: 0 4px 16px rgba(0, 0, 0, 0.12), 0 2px 4px rgba(0, 0, 0, 0.06);
  --vd-shadow-glow: 0 0 0 1px var(--vscode-focusBorder), 0 2px 8px rgba(0, 0, 0, 0.15);
...
.card--inset {
  background: var(--vd-surface-overlay);
  border: 1px solid var(--vd-border-subtle);
...
.vd-section::after {
  ...
  position: absolute;
  bottom: 0;
  left: var(--vd-space-4);    /* 16px inset */
  right: var(--vd-space-4);
  height: 1px;
...
.vd-section--error    { border-left-color: var(--vd-runtime); }
.vd-section--fix      { border-left-color: var(--vd-success); }
.vd-section--summary  { border-left-color: var(--vd-logic); }
```
...
.vd-disclosure-trigger:hover {
  color: var(--vscode-textLink-foreground);
}

/* Chevron indicator */
.vd-disclosure-trigger::before {
  content: '';
...
details[open] > .vd-disclosure-trigger::before {
  transform: rotate(45deg);
}

.vd-disclosure-body {
  padding: var(--vd-space-3) 0 var(--vd-space-1) 0;
...
.stat-card {
  ...
  border: 1px solid var(--vd-border-subtle);
  border-radius: var(--vd-radius-md);
  padding: var(--vd-space-3);      /* 12px -- compact */
  text-align: left;
  position: relative;
  min-height: 72px;
  display: flex;
...
.stat-card .stat-value {
  font-size: var(--vd-text-xl);     /* 22.5px */
  font-weight: var(--vd-weight-bold);
  line-height: 1;
  margin-bottom: 2px;
  font-variant-numeric: tabular-nums;
...
.vd-achievements {
  ...
  gap: var(--vd-space-2);    /* 8px -- tighter than current 10px */
}
...
.vd-achievement {
  ...
  align-items: center;
  text-align: center;
  padding: var(--vd-space-3) var(--vd-space-2);   /* 12px 8px */
  border-radius: var(--vd-radius-md);
  border: 1px solid var(--vd-border-subtle);
  background: var(--vd-surface-raised);
...
.vd-achievement.unlocked:hover {
  transform: translateY(-2px);
  box-shadow: var(--vd-shadow-md);
}

.vd-achievement.locked {
  opacity: 0.4;
...
:root {
  /* ── Typography ── */
  --vd-font-sans:  'Inter', var(--vscode-font-family, 'Segoe UI', system-ui, sans-serif);
  --vd-font-mono:  var(--vscode-editor-font-family, 'JetBrains Mono', 'Fira Code', 'Consolas', monospace);
  ...
  --vd-leading-snug:    1.4;
  --vd-leading-normal:  1.6;
  --vd-leading-relaxed: 1.75;

  /* ── Letter Spacing ── */
  --vd-tracking-tight:  -0.02em;
  --vd-tracking-normal:  0;
  --vd-tracking-wide:    0.04em;
  --vd-tracking-wider:   0.06em;
  ...
  /* ── Transitions ── */
  --vd-transition-fast:    0.1s ease;
  --vd-transition-normal:  0.15s ease;
  --vd-transition-slow:    0.25s ease;
}
```
...
| `--vd-gap: 16px` | 10 spacing tokens (`--vd-space-1` through `--vd-space-12`) | Granular 4px grid for dense sidebar |
| `--vd-radius: 4px` | 5 radius tokens (`--vd-radius-sm` through `--vd-radius-full`) | Different radii for different element sizes |
| `--vd-transition: 0.1s ease-in-out` | 3 transition tokens | Different speeds for different interaction types |
...
| `h1 { font-size: 1.5em }` | `h1 { font-size: var(--vd-text-xl) }` (1.728rem = 22.5px) | Minor Third scale harmony |
